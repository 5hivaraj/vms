...
const formatAssessment = (visitor) => {
  if (visitor.assessmentPassed === null) return 'N/A';
  return visitor.assessmentPassed
    ? `Pass (${visitor.assessmentScore}/${visitor.assessmentTotal})`
    : `Fail (${visitor.assessmentScore}/${visitor.assessmentTotal})`;
};
...
const formatRow = (visitor) => ({
  ...
  assessment: formatAssessment(visitor),
...
export const exportToPDF = (visitors) =>
  new Promise((resolve, reject) => {
    ...
    const headers = ['Token', 'Name', 'Mobile', 'Company', 'Assessment', 'Date', 'Time'];
    const colWidths = [50, 100, 80, 100, 80, 70, 70];
    let y = doc.y;
    ...
    visitors.forEach((visitor) => {
      ...
      x = 40;
      [row.token, row.name, row.mobile, row.company, row.assessment, row.date, row.time].forEach(
        (cell, i) => {
