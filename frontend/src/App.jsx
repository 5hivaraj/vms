...
import Assessment from './pages/kiosk/Assessment';
...
function App() {
  // Hide admin only in the installed Android APK, not in the PC browser.
  const kioskOnly = isCapacitorNative();
  useEffect(() => {
    if (kioskOnly) return undefined;
        ...
        <KioskProvider>
          <BrowserRouter>
            <Routes>
              ...
              <Route
                path="*"
                element={<Navigate to={kioskOnly ? '/' : '/'} replace />}
              />
            </Routes>
          </BrowserRouter>
        </KioskProvider>
