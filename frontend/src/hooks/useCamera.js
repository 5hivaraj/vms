...
const getInsecureContextMessage = () => {
  const { protocol, hostname } = window.location;
  if (protocol === 'https:' || hostname === 'localhost' || hostname === '127.0.0.1') {
    return null;
  }
  const httpsUrl = `https://${hostname}${window.location.port ? `:${window.location.port}` : ''}`;
  return `Camera is blocked on ${protocol}//${hostname}. Open the kiosk using ${httpsUrl} instead (accept the certificate warning).`;
};

const getErrorMessage = (err) => {
  const insecureMessage = getInsecureContextMessage();
  if (insecureMessage) return insecureMessage;
...
const requestStream = async () => {
  const insecureMessage = getInsecureContextMessage();
  if (insecureMessage) {
    throw Object.assign(new Error(insecureMessage), { name: 'SecurityError' });
  }
